/**
 * Default parameter rule templates.
 * Mirrors Python graphon.model_runtime.entities.defaults.PARAMETER_RULE_TEMPLATE.
 */

import { DefaultParameterName } from './model.js'
import type { I18nObject } from './common.js'

interface ParameterRuleTemplate {
  label: I18nObject
  type: string
  help?: I18nObject
  required: boolean
...
export const PARAMETER_RULE_TEMPLATE: Record<string, ParameterRuleTemplate> = {
  ...
  [DefaultParameterName.PRESENCE_PENALTY]: {
    label: { en_US: 'Presence Penalty', zh_Hans: '存在惩罚' },
    type: 'float',
    help: {
  ...
  [DefaultParameterName.FREQUENCY_PENALTY]: {
    label: { en_US: 'Frequency Penalty', zh_Hans: '频率惩罚' },
    type: 'float',
    help: {
  ...
  [DefaultParameterName.RESPONSE_FORMAT]: {
    label: { en_US: 'Response Format', zh_Hans: '回复格式' },
    type: 'string',
    help: {
